...
function AddProjectModalHelper({
  ...
  useEffect(() => {
    if (debouncedDomain.length > 0) {
      fetch(`/api/projects/hussain.fun/domains/${debouncedDomain}/exists`).then(
        async (res) => {
          if (res.status === 200) {
            const exists = await res.json();
            ...
            {domainError &&
              (domainError === "Domain is already in use." ? (
                <p className="mt-2 text-sm text-red-600" id="domain-error">
                  ...
                  <a
                    className="underline"
                    href="mailto:hussain@hussain.fun?subject=My Domain Is Already In Use"
                  >
                    Contact us
                  </a>{" "}
